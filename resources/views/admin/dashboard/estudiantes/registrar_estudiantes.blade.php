@extends('admin.layouts.app')

@section('content')
    <div class="page-inner">
        <div class="page-header">
            <h4 class="page-title">Registrar Estudiantes</h4>
            <ul class="breadcrumbs">
                <li class="nav-home">
                    <a href="#">
                        <i class="flaticon-home"></i>
                    </a>
                </li>
                <li class="separator">
                    <i class="flaticon-right-arrow"></i>
                </li>
                <li class="nav-item">
                    <a href="#">Estudiantes</a>
                </li>
                <li class="separator">
                    <i class="flaticon-right-arrow"></i>
                </li>
                <li class="nav-item">
                    <a href="#">Registrar Estudiantes</a>
                </li>
            </ul>
        </div>

        <div class="row justify-content-center">
            <div class="col-12 col-lg-8">
                <div class="card">
                    <div class="card-body">
                        <div class="row justify-content-center">
                            <!--Formulario de registro de estudiantes por medio de campos-->
                            <form action="{{ route('guardarEstudiantes.store') }}" method="POST"
                                enctype="multipart/form-data" class="needes-validation" novalidate>
                                @csrf <!-- Protección CSRF de Laravel -->
                                <h1 class="text-center">Añadir EStudiante</h1>
                                <!-- Campo de nombre del estudiante -->
                                <div class="form-group">
                                    <label class="form-label" for="text">Nombre <span
                                            class="text-danger">*</span></label>
                                    <input type="text"
                                        class="form-control @error('nombre_estudiante') is-invalid @enderror"
                                        name="nombre_estudiante" id="nombre_estudiante"
                                        value="{{ old('nombre_estudiante') }}" maxlength="100"
                                        placeholder="Ingrese el nombre" required>
                                    @error('nombre_estudiante')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <!-- Campo del apellido del estudiante -->
                                <div class="form-group">
                                    <label class="form-label" for="text">Apellido <span
                                            class="text-danger">*</span></label>
                                    <input type="text"
                                        class="form-control @error('apellido_estudiante') is-invalid @enderror"
                                        name="apellido_estudiante" id="apellido_estudiante"
                                        value="{{ old('apellido_estudiante') }}" maxlength="100"
                                        placeholder="Ingrese el apellido" required>
                                    @error('apellido_estudiante')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <!-- Campo del código estudiantil del estudiante -->
                                <div class="form-group">
                                    <label class="form-label" for="text">Código estudiantil <span
                                            class="text-danger">*</span></label>
                                    <input type="number"
                                        class="form-control @error('codigo_estudiante') is-invalid @enderror"
                                        name="codigo_estudiante" id="codigo_estudiante"
                                        value="{{ old('codigo_estudiante') }}" maxlength="100"
                                        placeholder="Ingrese el código" required>
                                    @error('codigo_estudiante')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <!-- Campo del email  del estudiante -->
                                <div class="form-group">
                                    <label class="form-label" for="text">Correo institucional <span
                                            class="text-danger">*</span></label>
                                    <input type="email"
                                        class="form-control @error('correo_estudiante') is-invalid @enderror"
                                        name="correo_estudiante" id="correo_estudiante"
                                        value="{{ old('correo_estudiante') }}" maxlength="100"
                                        placeholder="Ingrese el correo" required>
                                    @error('correo_estudiante')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <!-- Campo de la contraseña del estudiante -->
                                <div class="form-group">
                                    <label class="form-label" for="text">Contraseña <span
                                            class="text-danger">*</span></label>
                                    <input type="password"
                                        class="form-control @error('contraseña_estudiante') is-invalid @enderror"
                                        name="contraseña_estudiante" id="contraseña_estudiante"
                                        value="{{ old('contraseña_estudiante') }}" maxlength="100"
                                        placeholder="Ingrese la contraseña" required>
                                    @error('contraseña_estudiante')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <!-- Programa Académico -->
                                <div class="form-group">
                                    <label class="form-label" for="text">Programa académico <span
                                            class="text-danger">*</span></label>
                                    <select class="form-control @error('programa_id') is-invalid @enderror"
                                        name="programa_id" id="programa_id" value="{{ old('programa_id') }}"
                                        maxlength="100" required>
                                        @foreach ($programas as $programa)
                                            <option value="{{ $programa->id_programa}}">{{ $programa->nombre_programa}}</option>
                                        @endforeach
                                    </select>
                                    @error('programa_id')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <!-- Botones -->
                                <div class="form-group text-center">
                                    <!-- Boton para cargar un formulario manualmente -->
                                    <button type="submit" class="btn btn-primary">Cargar Estudiante</button>
                                    <!-- Boton de cargar formulario mediante archivo CSV -->
                                    <button type="button" class="btn btn-primary" data-toggle="modal"
                                        data-target="#cargaMasivaModal">
                                        Carga masiva
                                    </button>
                                </div>
                                <!-- Modal para cargar un archivo CSV -->
                                <div class="modal fade" id="cargaMasivaModal" tabindex="-1" role="dialog"
                                    aria-labelledby="cargaMasivaModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="cargaMasivaModalLabel">Carga masiva de un
                                                    estudiante</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>

                                            <div class="modal-body">
                                                <h2 class="text-center">Carga Masiva</h2>
                                                <!-- Campo para seleccionar un archivo CSV -->
                                                <div class="form-group">
                                                    <label for="archivo_programa">Selecciona un archivo CSV</label>
                                                    <div class="input-group">
                                                        <input type="file" class="form-control" id="archivo_estudiante"
                                                            name="archivo_estudiante" accept=".csv"
                                                            onchange="toggleFields(this)">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <!-- Boton cerrar modal -->
                                                <button type="button" class="btn btn-primary"
                                                    data-dismiss="modal">Close</button>
                                                <!-- Boton guardar un archivo  CSV -->
                                                <button type="submit" class="btn btn-primary">Guardar</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
